/**
 * APP_LOPCOW.JS - Toko Super Komputer
 * Sistem Pendukung Keputusan Pemilihan Laptop (10 Kriteria)
 * Kombinasi Metode LOPCOW (Logarithmic Percentage Change-driven Objective Weighting) & TOPSIS
 */
...
function getLopcowInitialKriteriaList() {
  return [
...
function spkLopcowApp() {
  return {
    ...
    kriteriaList: getLopcowInitialKriteriaList(),
    laptopsData: [],
    hasilRanking: [],
    matriksData: null,
    lopcowData: null,
    ...
    // 2) Rata-rata r_bar_j = (1/n) * SUM(r_ij)
    // 3) Deviasi Logaritmik LP_ij = [ ln( (r_ij + sigma) / (r_bar_j + sigma) ) ]^2  (sigma = 0.01)
    // 4) Variansi Kriteria V_j = (1/n) * SUM(LP_ij)
    // 5) Bobot Akhir w_j = V_j / SUM(V_k)
    hitungBobotLopcow(dataset) {
      ...
      // 2. Normalisasi Min-Max Matriks r_ij
      const matrixR = [];
      data.forEach(row => {
        const rRow = { id: row.id, nama: row.nama };
        keys.forEach(key => {
          ...
          if (range === 0) {
            ...
            if (isCost) {
              rRow[key] = (minMax[key].max - val) / range;
            } else {
              rRow[key] = (val - minMax[key].min) / range;
            }
          }
        });
        matrixR.push(rRow);
      ...
      // 3. Rata-rata Normalisasi per kriteria (r_bar_j)
      const rBar = {};
      keys.forEach(key => {
        const sum = matrixR.reduce((acc, row) => acc + row[key], 0);
        rBar[key] = sum / n;
      });

      // 4. Deviasi Persentase Logaritmik (LP_ij) & Variansi (V_j)
      const matrixLP = [];
      const Vj = {};
      let totalV = 0;

      data.forEach(row => {
        const rRow = matrixR.find(r => r.id === row.id);
        const lpRow = { id: row.id, nama: row.nama };
        keys.forEach(key => {
          const r_ij = rRow[key];
          const r_bar = rBar[key];
          const ratio = (r_ij + sigma) / (r_bar + sigma);
          lpRow[key] = Math.pow(Math.log(ratio), 2);
        });
        matrixLP.push(lpRow);
      });

      keys.forEach(key => {
        const sumLP = matrixLP.reduce((acc, row) => acc + row[key], 0);
        Vj[key] = sumLP / n;
        totalV += Vj[key];
      });
      ...
      // 5. Hitung Bobot LOPCOW w_j
      this.kriteriaList.forEach((k, idx) => {
        const key = keys[idx];
        if (totalV > 0) {
          k.bobot = Vj[key] / totalV;
        } else {
          k.bobot = 1 / 10;
        }
        k.lopcow_V = Vj[key];
    ...
    kalkulasiTOPSIS(isUserClick = false) {
      ...
      // 1. Hitung bobot LOPCOW dari dataset aktif
      this.hitungBobotLopcow(dataToProcess);
    ...
    exportCSV() {
      ...
      link.setAttribute("download", `SPK_LOPCOW_TOPSIS_SuperKomputer_${new Date().toISOString().slice(0,10)}.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      this.showToast("File CSV hasil ranking LOPCOW + TOPSIS berhasil diunduh!", "success");
    },
...
if (typeof window !== 'undefined') {
  window.spkLopcowApp = spkLopcowApp;
}
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { spkLopcowApp };
}
